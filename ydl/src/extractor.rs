...
impl SubtitleExtractor {
    ...
    pub async fn discover_tracks(&self, video_id: &str) -> YdlResult<Vec<SubtitleTrack>> {
        ...
        if tracks.is_empty() {
            if let Ok(page_tracks) = self.discover_from_watch_page(video_id).await {
        ...
        if tracks.is_empty() {
            if let Ok(mobile_tracks) = self.discover_from_mobile_page(video_id).await {
        ...
        if tracks.is_empty() {
            if let Ok(api_tracks) = self.discover_from_api(video_id).await {
    ...
    pub async fn get_video_metadata(&self, video_id: &str) -> YdlResult<VideoMetadata> {
        ...
        if let Some(video_details) = &player_response.video_details {
            if let Some(length_str) = &video_details.length_seconds {
    ...
    async fn discover_from_api(&self, video_id: &str) -> YdlResult<Vec<SubtitleTrack>> {
        ...
        if let Some(player_response_str) = params.get("player_response") {
            if let Ok(player_response) = serde_json::from_str::<PlayerResponse>(player_response_str)
            {
